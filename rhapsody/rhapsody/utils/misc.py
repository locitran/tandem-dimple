...
def initialSetup(working_dir=None, refresh=False, download_EVmutation=True):
    ...
    print(f'You are running Rhapsody v{rd.__version__}')
    ...
    if working_dir is None:
        ...
        if type(old_dir) is str and os.path.isdir(old_dir):
            ...
            # pd.LOGGER.info('Pre-existing working directory detected: '
                        #    f'{working_dir}')
            print('Pre-existing working directory detected: '
                     f'{working_dir}')
        else:
            ...
            if os.path.isdir(working_dir):
                ...
                print(f'Default working directory set: {working_dir}')
    else:
        working_dir = os.path.abspath(working_dir)
        if os.path.isdir(working_dir):
            # pd.LOGGER.info(f'Working directory set: {working_dir}')
            print(f'Working directory set: {working_dir}')
        else:
    ...
    if os.path.isdir(folder) and not refresh:
        ...
        print(f'Pre-existing classifiers found: {folder}')
        ...
        print(f'Classifiers folder created: {folder}')
    ...
    if 'AUROC' in metrics:
        ...
        print(f'Pre-existing EVmutation metrics found.')
    else:
        ...
        if 'EVmut-DeltaE_epist' not in training_dataset.dtype.names:
            ...
            print('Unable to compute EVmutation metrics: '
                        'precomputed scores not found.')
        else:
            ...
            print(f'EVmutation metrics computed.')
    ...
    if type(folder) is str and os.path.isdir(folder):
        ...
        print(f'EVmutation folder found: {folder}')
    else:
        ...
        if os.path.isdir(DEFAULT_EVMUT_DIR):
            ...
            print(f'Downloading EVmutation data...')
            ...
            print(f'EVmutation folder set: {folder}')
        else:
            folder = None
    ...
    if which('dssp') is None and which('mkdssp') is None:
        ...
        print('DSSP is installed on the system.')
