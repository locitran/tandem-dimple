...
def main():
    ...
    run_dir = "/home/loci/main/tandem_website_dev/tandem/logs/TANDEM-DIMPLE-RYR1-90/20260211-2315-seed-0"
    files = sorted(glob.glob(os.path.join(run_dir, "model_fold_*", "transfer_metrics.csv")))

    rows = []
    for f in files:
        df = pd.read_csv(f)
        df["model_fold"] = os.path.basename(os.path.dirname(f))
        rows.append(df)
    ...
    # RYR1 (train_split)
    tf_ryr1_before = df[(df['split']=='train_split') & (df['stage']=='before')][['model_fold']+metrics].copy()
    tf_ryr1_before.rename(columns={m: f"test_{m}" for m in metrics}, inplace=True)

    tf_ryr1_after = df[(df['split']=='train_split') & (df['stage']=='after')][['model_fold']+metrics].copy()
    tf_ryr1_after.rename(columns={m: f"test_{m}" for m in metrics}, inplace=True)

    # R20000_test (before/after)
    r20000_before = df[(df['split']=='R20000_test') & (df['stage']=='before')][['model_fold']+metrics].copy()
    r20000_before.rename(columns={m: f"R20000_test_{m}" for m in metrics}, inplace=True)

    r20000_after = df[(df['split']=='R20000_test') & (df['stage']=='after')][['model_fold']+metrics].copy()
    r20000_after.rename(columns={m: f"R20000_test_{m}" for m in metrics}, inplace=True)

    # Merge into RYR1 tables
    tf_ryr1_before = tf_ryr1_before.merge(r20000_before, on="model_fold", how="left")
    tf_ryr1_after  = tf_ryr1_after.merge(r20000_after,  on="model_fold", how="left")
    ############################################################################################################
    run_dir = "/home/loci/main/tandem_website_dev/tandem/logs/TANDEM-DIMPLE-GJB2-90/20260211-2245-seed-73"
    files = sorted(glob.glob(os.path.join(run_dir, "model_fold_*", "transfer_metrics.csv")))
    ...
    metrics = ["accuracy", "auc", "precision", "recall", "f1"]
    tf_gjb2_before = df[(df['split']=='test') & (df['stage']=='before')][['model_fold']+metrics].copy()
    tf_gjb2_before.rename(columns={m: f"test_{m}" for m in metrics}, inplace=True)

    tf_gjb2_after = df[(df['split']=='test') & (df['stage']=='after')][['model_fold']+metrics].copy()
    tf_gjb2_after.rename(columns={m: f"test_{m}" for m in metrics}, inplace=True)
    ...
    # # Merge into RYR1 tables
    tf_gjb2_before = tf_gjb2_before.merge(r20000_before, on="model_fold", how="left")
    tf_gjb2_after  = tf_gjb2_after.merge(r20000_after,  on="model_fold", how="left")
    ############################################################################################################
