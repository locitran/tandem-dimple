"""
This file is copied from the OpenMM library and modified to reIndex chainID.
mpdbfile.py means "modified pdbfile.py".
"""
...
class PDBFile(object):
    ...
    # def writeModel(topology, positions, file=sys.stdout, modelIndex=None, keepIds=False, extraParticleIdentifier='EP'):
        ...
        def getChainName(chainIndex):
            if chainIndex < 26:
                return chr(ord('A') + chainIndex)
            elif chainIndex < 52:
                return chr(ord('a') + chainIndex - 26)
            elif chainIndex < 62:
                return str(chainIndex - 52)
            elif chainIndex < 77:
                # chanName_after_61 = {
                #     62: '~', 63: '!', 64: '@', 65: '#', 66: '%', 67: '-', 
                #     68: '&', 69: '*', 70: '_', 71: '+', 72: '=', 73: '|',
                ...
                return chanName_after_61[chainIndex]
            else:
                raise ValueError('Chain index is out of range')
        
        current_chainID = [c.id for c in topology.chains()]
        for (chainIndex, chain) in enumerate(topology.chains()):
            if keepIds and len(chain.id) == 1:
                ...
                while True:
                    ...
                    if chainName not in current_chainID:
                        break
                    chainIndex += 1
                current_chainID.append(chainName)
                LOGGER.warning(f'Chain {chain.id} is too long, rename it as {chainName}')
            else:
                chainName = getChainName(chainIndex)

            if modify_chain is not None:
                chainName = modify_chain
