...
class Timer:
    ...
    def track(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            timer_data["execution_time"].append(round(elapsed_time, 3))
            timer_data["average_execution_time"] = round(
                sum(timer_data["execution_time"]) / timer_data["n_calls"], 3)
            timer_data["total_execution_time"] = round(
                timer_data["total_execution_time"] + elapsed_time, 3)

            message = f"{func_name}() took {elapsed_time:.3f} s"
            if self.verbose:
                print(message)
            logger.info(message)
    ...
    def report(self, to_file=None):
        ...
        if to_file:
            with open(to_file, "w") as f:
                func_names = self.timers.keys()
                cols = ["Function", "n_calls", "total_time (s)", "avg_time (s)"]
                data = ''
                for col in cols:
                    data += f'{col:<20}'
                data += '\n'
                for name in func_names:
                    data += ''.join([
                        f'{name:<20}',
                        f'{self.timers[name]["n_calls"]:<20}',
                        f'{self.timers[name]["total_execution_time"]:<20}',
                        f'{self.timers[name]["average_execution_time"]:<20}\n'
                    ])
                f.write(data)
        return self.timers
